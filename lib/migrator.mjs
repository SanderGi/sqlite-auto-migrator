...
export class Migrator {
     ...
     */
    constructor(options = {}) {
        this.dbPath = path.resolve(options.dbPath) ?? path.join(process.cwd(), 'data.db');
        this.migrationsPath =
            path.resolve(options.migrationsPath) ?? path.join(process.cwd(), 'migrations');
        this.tempPath = path.join(this.migrationsPath, '/temp');
        this.schemaPath =
            path.resolve(options.schemaPath) ?? path.join(process.cwd(), 'schema.sql');
     ...
     * @param {Promise<any>} prompt a function to prompt the user for input
        ...
        for (const [oldname, newname] of renames) {
            ...
            if (action === Migrator.PROCEED) {
                ...
                manualMigrationReasons.push(
                    `${capitalizedUnit} "${oldname}" was potentially renamed to "${newname}"`,
                );
     ...
     * @private
     * @param {function} prompt a function to prompt the user for input
     * @param {Action} action the default action to take
