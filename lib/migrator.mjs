 ...
 * @property {string} [migrationTable] Name of the table to store migration information in. Default is `process.env.SAM_MIGRATION_TABLE` if provided, otherwise `migrations`
...
export class Migrator {
    ...
    constructor(options = {}) {
        this.dbPath =
            getAbsolutePath(options.dbPath) ??
            process.env.SAM_DB_PATH ??
            path.join(process.cwd(), 'data.db');
        this.migrationsPath =
            getAbsolutePath(options.migrationsPath) ??
            process.env.SAM_SCHEMA_PATH ??
            path.join(process.cwd(), 'migrations');
        this.tempPath = path.join(this.migrationsPath, '/temp');
        this.schemaPath =
            getAbsolutePath(options.schemaPath) ??
            process.env.SAM_MIGRATIONS_PATH ??
            path.join(process.cwd(), 'schema.sql');
        this.migrationTable =
            options.migrationTable ?? process.env.SAM_MIGRATION_TABLE ?? 'migrations';
    ...
    async make(keyargs = {}) {
        ...
        try {
            ...
            const { nameParts, pragmas, upStatements, downStatements, manualMigrationReasons } =
                await this.#diff(keyargs, oldDB, newDB, prompt);

            if (nameParts.length !== 0 || createIfNoChanges) {
                const nextId = Number(migrationFiles.length).toString().padStart(4, '0');
                await this.#writeMigrationFile(
                    nextId,
                    nameParts.join('__').substring(0, MAX_FILE_NAME_LENGTH) || 'not_implemented',
                    pragmas,
                    upStatements,
                    downStatements,
                );
     ...
     * @returns {Promise<{nameParts: string[], pragmas: Object, upStatements: string[], downStatements: string[], manualMigrationReasons: string[]}>} the diff results
     ...
     */
    async #diff(keyargs = {}, oldDB, newDB, prompt) {
        ...
        const [oldIndices, newIndices] = await Promise.all([getIndices(oldDB), getIndices(newDB)]);
    ...
    async status() {
        ...
        try {
            ...
            try {
                ...
                // to make actualDB match the schema of db, we must first take care of virtual tables since they introduce shadow tables
                const virtualTables = await getVirtualTables(db);
                for (const [_, sql] of virtualTables.entries()) {
                    await actualDB.run(sql);
                }
                const tablenames = await getTables(actualDB);
                for await (const { name, sql } of db.each(
                    'SELECT name, sql FROM sqlite_master ORDER BY rowid',
                )) {
                    if (
                        sql &&
                        name !== this.migrationTable &&
