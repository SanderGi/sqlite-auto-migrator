...
${downStatements.map(s => '    ' + s.trim() + ';').join('\n')}
    ...
    async #makeTables(
        ...
        for (const tableName of modifiedTableNames) {
            ...
            for (const [col, values] of newColumns.entries()) {
                const { notnull, dflt_value } = values;
                if (notnull && !dflt_value) {
                    manualMigrationReasons.push(
                        `Column "${col}" of table "${tableName}" has NOT NULL constraint without a default value`,
                    );
                }
            }
