...
export class Migrator {
    ...
    constructor(options = {}) {
        ...
        if (fsSync.existsSync(this.configPath)) {
            const config = JSON.parse(fsSync.readFileSync(this.configPath, 'utf8'));
    ...
    async make(keyargs = {}, log = s => process.stdout.write(s)) {
        ...
        try {
            ...
            const { nameParts, pragmas, upStatements, downStatements, manualMigrationReasons } =
                await this.#diff(keyargs, oldDB, newDB, prompt, log);
            ...
            if (manualMigrationReasons.length > 0) {
                console.log('\nManual migration required for the following reasons:');
     ...
     */
    async #diff(keyargs = {}, oldDB, newDB, prompt, log = () => {}) {
    ...
    async #makeTables(
        ...
        for (const tableName of modifiedTableNames) {
            ...
            for (const columnName of addedColumns) {
                ...
                const old = oldColumns.get(columnName);
                if (old && old.notnull && !old.dflt_value) {
                    const action = await this.#promptForAction(
                        prompt,
                        onDestructiveChange,
                        `Column "${columnName}" of table "${tableName}" had a NOT NULL constraint without a default value so migration can't be unapplied. Type "y"/"m" to require manual migration and "n" to ignore: `,
                    );
                    if (
                        action === Migrator.REQUIRE_MANUAL_MIGRATION ||
                        action === Migrator.PROCEED
                        ...
                        manualMigrationReasons.push(
                            `Column "${columnName}" of table "${tableName}" had a NOT NULL constraint without a default value`,
                        );
                    } else if (action !== Migrator.SKIP) {
                        throw new ValidationError(`Invalid action: ${action}`);
                    }
