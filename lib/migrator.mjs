...
export class Migrator {
    ...
    async #copySchema(srcDB, targetDB) {
        ...
        const virtualTables = await getVirtualTables(srcDB);
        for (const [_, sql] of virtualTables.entries()) {
            await targetDB.run(sql);
        }
        const tablenames = await getTables(targetDB);
    ...
    async #createMigrationDirectory() {
        await fs.mkdir(this.migrationsPath).catch(e => {
            if (e.code !== 'EEXIST') throw e;
        });
        // ensure it is readable and writable
        await fs.access(this.migrationsPath, fs.constants.R_OK | fs.constants.W_OK);
    }
